...
from .ranking_of_page import page_ranking as ranking
from .textcleaner import sentence_cleaning as sentence_wise_cleaning
from .similars import generate_hierarchy as generate_hierarchy
from .similars import remove_unwanted_words as remove_unwanted_words
...
def summarize(text, ratio=0.2, words=None, language="english", split=False, scores=False, additional_stopwords=None):
    ...
    sentences = sentence_wise_cleaning(text, language, additional_stopwords)

    # Creates the graph and calculates the similarity coefficient for every pair of nodes.
    graph = generate_hierarchy([sentence.token for sentence in sentences])
    _set_graph_edge_weights(graph)

    # Remove unwanted words.
    remove_unwanted_words(graph)

    if len(graph.nodes()) == 0:
        return [] if split else ""

    page_ranks = ranking(graph)

    # Adds the summa scores to the sentence objects.
    _add_scores_to_sentences(sentences, page_ranks)
...
def get_graph(text, language="english"):
    sentences = sentence_wise_cleaning(text, language)
